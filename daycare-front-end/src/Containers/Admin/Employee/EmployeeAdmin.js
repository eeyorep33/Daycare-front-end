...
import { getEmployees, editEmployee, deleteEmployee, addEmployee } from '../../../Actions/EmployeeActions'
...
class EmployeeAdmin extends Component {
  ...
  componentDidMount() {
    localStorage.setItem('route', '/admin/employee');    
    this.props.getEmployees()
    console.log("error")
    console.log(this.props.employeeError)
  }
  ...
  openModal(i, employee, type) {  
    if(type === "edit") {
      this.setState(prevState => ({
        open: true,
        index: i,
       employee: {...prevState.employee, name: employee.name,      
...
this.setState(prevState => ({
  employee: {...prevState, name:'', email:'', image: '', is_admin: 'false', id: ''}
}))
  }
  ...
  editEmployee() {
    ...
    this.props.editEmployee(formData, this.state.employee.id)
...
this.closeModal("delete") 
    ...
    render() {
      ...
      this.state.employee.image.name : 'Upload Image Here'
      let employeeMap = null;
      if (this.props.employees) {      
        employeeMap = (
         
            <div>
            {this.props.employees.sort(this.naturalCompare).map((employee, i) => {
             
              return (
               <React.Fragment  key={employee.id}>               
                 <Grid container spacing={3}  className={classes.buttonContainer}  key={employee.id}>
                   <Grid className={classes.mobile} item xs={12} sm={6} key={employee.id}>              
                    <Typography
                    color="secondary"
                      variant="h4"
                    ...
                    <Button
                      ...
                      className={classes.button}
                      color="secondary"
                      onClick={() => this.openModal(employee.id, employee.name, "delete")}
                    >
                      Delete
                    </Button>               
                  </Grid>
      ...
      open={this.props.employeeError !== null ? true : false} 
...
className={classes.textContainer}>
  ...
  onChange={e => this.updateContent(e.target.value, e.target.name)} 
  label="Employee Name" 
...
<Grid item xs={12} className={classes.textContainer}>
  <TextField  className={classes.root}
   ...
   value={this.state.employee.email}></TextField>
</Grid>
<Grid item container style={{marginTop: "2rem", marginBottom: "4rem"}}>
<Grid item xs={12} sm ={6} className={classes.radioContainer}>
    <Typography  color="secondary" className={classes.titleMargin}>Has Administrative Rights</Typography>
</Grid>
<Grid item xs={12} sm ={6} className={classes.radioContainer}>
...
<FormControl>
          ...
          name="admin"
           onChange={(e)=> this.updateContent(e.target.value, "admin")}>
...
<Grid item xs={12}>
<Button 
disabled={this.state.nameHelper.length !== 0 || this.state.emailHelper.length !== 0}
type="submit" 
...
const mapStateToProps = state => {
    ...
    const mapDispatchToProps = dispatch => {
      return {
      getEmployees: (facilityId) => dispatch(getEmployees(facilityId)),
      addEmployee: (employee) => dispatch(addEmployee(employee)),
      editEmployee: (employee, id) => dispatch(editEmployee(employee, id)),
