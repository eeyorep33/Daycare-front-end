      ...
      classMap = (
        <div>
                  ...
                  >
                    ...
                    <Grid item xs={12} sm={6} className={classes.mobile}>
                      ...
                      <Button1
                        ...
                        color="secondary"
                        onClick={() =>
                          this.openModal(classroom.id, classroom.name)
                        }
                      >
                        Delete
                      </Button1>
          ...
          <Modal
            ...
            change={(e) =>
              this.updateclassroomName(e.target.value, e.target.name)
            }
            update={(e) => {
              this.editClassroom(e, this.state.index);
            }}
          ...
          <DeleteModal
            ...
            close={() => this.closeModal('delete')}
            name={this.state.classroomName}
            delete={() => this.deleteClassroom()}
            type={'Classroom'}
          />
        </div>
      );
...
const mapStateToProps = (state) => {
  return {
    ...
    classError: state.classReducer.classError,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    addClassroom: (token, classroom, user, facility) =>
      dispatch(addClassroom(token, classroom, user, facility)),
    editClassroom: (classroom, id, facility, user, token) =>
      dispatch(editClassroom(classroom, id, facility, user, token)),
    updateMenu: (index, name) =>
      dispatch({ type: 'UPDATE_MENU', values: { index: index, name: name } }),
    clearError: () => dispatch({ type: 'CLEAR_CLASS_ERROR' }),
    deleteClassroom: (id, token, facility, user) =>
      dispatch(deleteClassroom(id, token, facility, user)),
  };
};
